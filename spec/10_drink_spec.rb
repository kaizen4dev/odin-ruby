...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:lemonade) { described_class.new('lemonade') }

      it 'is your choice of beverage' do
        expect(lemonade.type).to be 'lemonade'
      end

      it 'has 16 ounces' do
        expect(lemonade).to be_full
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:cola) { described_class.new('cola') }

      it 'is full' do
        expect(cola).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:juice) { described_class.new('juice', 5) }

      it 'is not full' do
        expect(juice).not_to be_full
      end
    end
  end
end
